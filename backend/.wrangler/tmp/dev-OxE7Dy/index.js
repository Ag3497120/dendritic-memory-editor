...
// .wrangler/tmp/bundle-biKnB1/checked-fetch.js
...
// src/auth/jwt.ts
var createToken = /* @__PURE__ */ __name(async (payload, env) => {
...
google.get("/callback", async (c) => {
  ...
  if (existingUsersResult.results.length > 0) {
    ...
    await db.prepare("INSERT INTO users (id, provider, provider_id, username, is_expert) VALUES (?, 'google', ?, ?, ?)").bind(userId, googleUser.sub, googleUser.name, 0).run();
...
github.get("/callback", async (c) => {
  ...
  if (existingUsersResult.results.length > 0) {
    ...
    const userId = v4_default();
    await db.prepare("INSERT INTO users (id, provider, provider_id, username, is_expert) VALUES (?, 'github', ?, ?, ?)").bind(userId, githubUser.id.toString(), githubUser.login, 0).run();
    const newUserResult = await db.prepare("SELECT * FROM users WHERE id = ?").bind(userId).first();
    user = newUserResult;
  }
  if (!user || typeof user.id !== "string" || typeof user.is_expert !== "number") {
...
guest.post("/login", async (c) => {
  const db = getDb(c);
  const userId = v4_default();
  const guestProviderId = `guest_${v4_default()}`;
...
// src/routes/auth/npi.ts
var npi = new Hono2();
async function verifyNpiWithNPPES(npi2, nppesApiBaseUrl) {
  if (!nppesApiBaseUrl) {
    console.warn("NPPES_API_BASE_URL is not set. Skipping NPI verification.");
    return { isValid: false };
  }
  try {
    const response = await fetch(`${nppesApiBaseUrl}?number=${npi2}&version=2.1`);
    if (!response.ok) {
    ...
    return { isValid: true, providerName: providerName || `NPI User ${npi2}` };
  } catch (error) {
    console.error("Error calling NPPES API:", error);
    return { isValid: false };
  }
}
__name(verifyNpiWithNPPES, "verifyNpiWithNPPES");
npi.post("/verify", async (c) => {
  const { npiNumber } = await c.req.json();
...
npi.get("/status", async (c) => {
  ...
  try {
    ...
    const { npi: npi2, is_expert } = result;
    return c.json({ npi: npi2 || null, is_expert: is_expert === 1 });
  } catch (e) {
...
var authMiddleware = createMiddleware(async (c, next) => {
  ...
  c.set("user", { userId: payload.userId, isExpert: payload.isExpert });
  await next();
});
...
tiles.post("/", async (c) => {
  ...
  const { content, domain } = await c.req.json();
  if (!content || !domain) {
    return c.json({ error: "Content and domain are required." }, 400);
  }
  const db = getDb(c);
  const tileId = v4_default();
  ...
  try {
    ...
    for (const tile of iathData.tiles) {
      try {
        ...
        const z = coords.z !== void 0 ? coords.z : Math.random() * 100;
...
app.route("/api/auth/google", google_default);
app.route("/api/auth/github", github_default);
app.route("/api/auth/orcid", orcid_default);
app.route("/api/auth/guest", guest_default);
app.route("/api/auth/npi", npi_default);
app.route("/api/tiles", tiles_default);
var src_default = app;
...
// .wrangler/tmp/bundle-biKnB1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-biKnB1/middleware-loader.entry.ts
